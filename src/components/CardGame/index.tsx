...
const validationSchema = yup.object().shape({
  homeScore: yup.number().required('Placar para time da casa faltando!').integer('integer'),
  awayScore: yup.number().required('Placar para time da convidado faltando!').integer()
})
      ...
      await axios({
        ...
        .then(res => {
          ...
          if (error.code === 'ERR_NETWORK') {
            enqueueSnackbar(`Ops, api não está respondendo...`,
              {
                variant: 'error',
                persist: false
              })
